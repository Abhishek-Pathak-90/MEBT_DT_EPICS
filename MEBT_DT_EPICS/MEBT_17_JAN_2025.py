...
#    ADDED / DEBUG: We will print the dictionary keys to confirm channel names
# -----------------------------------------------------------------------------
...
# ADDED / DEBUG: Print out keys to ensure we have matching PVs
...
# ADDED / DEBUG: Short sleep to allow channels to connect
time.sleep(1.0)
...
def run_pipeline_and_get_rms(
    ...
    # ADDED / DEBUG: We'll ensure the arrays match in length
    n_bpm_list = len(bpm_list)
    if len(x_rms) < n_bpm_list or len(y_rms) < n_bpm_list:
        print(f"WARNING: x_rms or y_rms array is shorter ({len(x_rms)}) than bpm_list ({n_bpm_list}).")
        print("Indices may go out of range. Check your lattice vs BPM definitions.")

    for ndx in range(n_bpm_list):
        ...
        if bpm_name in bpmxrms_dict and bpm_name in bpmyrms_dict:
            ...
            print(f"WARNING: {bpm_name} not in bpmxrms_dict or bpmyrms_dict. Skipping putFloat.")
        if bpm_name in bpmxpos_dict and bpm_name in bpmypos_dict:
            ...
            print(f"WARNING: {bpm_name} not in bpmxpos_dict or bpmypos_dict. Skipping putFloat.")
...
def main():
    ...
    if len(bpm_positions) > 0:
        ...
        # ADDED / DEBUG: If you want to write these to PVs again, do so here:
